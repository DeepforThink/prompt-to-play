...
- `delivery.json`: selected revision, `passed` or `best_effort` delivery mode, evaluation hash, failed checks, remaining issues, and correction-stop evidence;
...
6. **Repeat and select** — validate every patch, rebuild from the resulting complete WorldSpec, and reevaluate. Permit at most the policy's two correction rounds after the initial build. A fully passing revision outranks every failing revision. Otherwise, a revision that passes every non-visual hard check outranks any structurally failing revision; within the same delivery class choose the highest weighted score. Launch the selected structurally valid revision and record it as `best_effort` when its visual gate or weighted threshold still fails. Reject launch only when no revision passes the structural delivery checks. If a RepairAgent proposal is invalid, record the rejected correction and select from revisions that already completed evaluation.
