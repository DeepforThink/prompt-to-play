...
- `code_objects.json`: CodeObjectAgent status, assigned stable IDs, generated source hash, and sanitized fallback reason;
...
2. **Execute** — compile the WorldSpec deterministically. A bounded CodeObjectAgent may generate a fixed-wrapper C# method for up to 16 buildings, props, and interactables; the host rejects unsafe APIs and retains the built-in implementation as a compile fallback. Generate regions, roads, lights, cameras, player spawn, objectives, and exit from their arrays rather than fixed names or counts. Save, reload, and verify the scene before declaring the build successful.
