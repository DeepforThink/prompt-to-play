...
- A CodeObjectAgent that writes prompt-specific Godot C# geometry for regions, roads, buildings, props, and interactables. Terrain, road, and placed-object requests are isolated so one oversized or invalid response does not erase the other generated segments; unsafe output is rejected, and compile failures automatically restore the built-in implementation.
- Prompt-derived runtime presentation: racing briefs receive a visible third-person vehicle, driving controls, a connected circuit with start-line extension, and reduced-width map access roads. Topographic briefs receive contour terraces, a paper backdrop, map-aware boundary visibility, and camera poses adapted to world bounds.
