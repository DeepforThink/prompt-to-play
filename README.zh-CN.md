...
- **可审计证据** —— 每个版本均记录清单、哈希、耗时、模型调用信息、日志、截图、评分与最终选择结果。
...
Windows 启动器会以掩码方式读取 API 密钥。请通过命令行参数选择服务商和模型：
...
Prompt-to-Play 会保存每个修订版本的源码快照和机器可读证据，包括构建日志、结构与交互报告、渲染截图、视觉反馈、耗时、哈希以及最终版本选择。这些记录让失败问题可以定位，也让通过的结果可以追溯。

候选版本只有通过所需的构建、结构、交互、截图和视觉门禁后才会被接受。精美截图不能掩盖构建或交互失败；同样，仅能成功编译但缺少指定玩法或视觉可读性的场景也不能通过。
...
欢迎参与改进项目。开发环境、测试要求、安全规范与 Pull Request 约定请参阅 [CONTRIBUTING.md](CONTRIBUTING.md)。
